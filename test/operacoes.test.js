...
describe('Suíte de Testes Fraca para 50 Operações Aritméticas', () => {
  ...
  test('6. deve calcular a raiz quadrada de um quadrado perfeito e lançar erro para números negativos', () => {
    expect(raizQuadrada(16)).toBe(4);
    expect(raizQuadrada(0)).toBe(0);
    expect(() => raizQuadrada(-5)).toThrow('Não é possível calcular a raiz quadrada de um número negativo.');
  });
  ...
  test('37. deve verificar se um número é divisível por outro', () => {
    ...
    expect(isDivisivel(2, 10)).toBe(false);
  });
